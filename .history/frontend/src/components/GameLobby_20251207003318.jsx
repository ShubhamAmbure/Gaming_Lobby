...
// {/* Mobile Search Drawer (Optional) */}
// {mobileSearchOpen && (
//   <div className="md:hidden fixed top-0 left-0 w-full p-4 flex items-center gap-2 bg-white dark:bg-slate-800 shadow z-[9999]">
...
//   className={`md:hidden p-2 rounded-lg relative z-[9999] ${
//     isDark ? 'bg-slate-700 text-white' : 'bg-slate-200 text-black'
//   }`}
// >
...
//         {/* ===================== UPDATED GAME GRID (BIGGER CARDS) ===================== */}
//         <div
//           ref={gamesRef}
...
const GameLobby = ({ games = [] }) => {
  ...
  const currentLangMeta = languageMeta[i18n.language] || { label: 'EN', flag: '🌐' };
  ...
  const currentCurrencyMeta =
    currencyMeta[currency || 'USD'] || { label: 'USD', flag: '🇺🇸' };
        ...
        <div className="flex items-center justify-between px-4 py-4 w-full">
          ...
          <div className="flex items-center gap-3 md:gap-4">
                ...
                >
                  {['en', 'es', 'fr'].map((lang) => (
                    <button
                      ...
                      className="block px-4 py-2 w-full text-left hover:bg-slate-200 dark:hover:bg-slate-700"
                    >
                      {languageMeta[lang].flag} {languageMeta[lang].label}
                    </button>
                  ))}
            ...
            <div className="relative">
              <button
                onClick={() => setUserDropdown(!userDropdown)}
                className="w-10 h-10 rounded-full flex items-center justify-center bg-gradient-to-r from-purple-500 to-blue-500 text-white shadow-md"
              >
                <User className="w-5 h-5" />
              </button>
