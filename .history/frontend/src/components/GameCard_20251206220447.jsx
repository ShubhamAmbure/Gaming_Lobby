...
//     navigate(`/game/${game.id}`);
//   }, [game, navigate, addRecent, addNotification, addToast]);

//   const handleCardClick = (e) => {
//     if (!isMobile) return handlePlay();
...
//     >
//       <div
//         className={`relative w-full overflow-hidden rounded-xl shadow-lg transition-transform duration-300 
...
//               <h3 className="text-xl sm:text-2xl font-bold">{game.title}</h3>
...
//               className="p-2 rounded-full bg-white/30 backdrop-blur-md hover:scale-125 transition-all"
//             >
//               <Heart
//                 className="w-6 h-6"
//                 style={{
...
const GameCard = ({ game }) => {
  ...
  // -------------------------------
  // FIX 1 — DO NOT USE window DURING RENDER (causes blank screen)
  // -------------------------------
  ...
  // -------------------------------
  // FIX 2 — Mobile click logic (prevent blank UI)
  // -------------------------------
  const handleCardClick = (e) => {
    if (!isMobile) return handlePlay();
  ...
  const toggleFavoriteHandler = (e) => {
    ...
    addToast({
      id: Date.now(),
      message: isFavorite ? "❤️ Removed from Favorites" : "✨ Added to Favorites",
  ...
  // -------------------------------
  // FIX 3 — Safe image fallback (Safari crash fix)
  // -------------------------------
  const safeImage = game.photo || "/images/fallback.jpg";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Title Section */}
        <div
          className={`absolute inset-0 p-4 sm:p-5 z-10 flex flex-col justify-between transition-all duration-300
          ${showInfo ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`}
        >
          <div className="flex justify-between items-start">
            <div className="text-white drop-shadow-lg">
        ...
        {/* Bottom Info */}
        <div
          className={`absolute inset-x-0 bottom-0 p-4 sm:p-6 bg-black/70 backdrop-blur-md z-20 transition-all duration-300
          ${showInfo ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
        >
          <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 text-white">
            <div>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl text-white font-semibold bg-gradient-to-r from-blue-300 to-blue-500 hover:from-blue-400 hover:to-blue-600 flex items-center justify-center gap-2 shadow-md hover:scale-105 active:scale-95 transition-all"
          >
            <Play className="w-5 h-5" />
            <span>Play</span>
          </button>
